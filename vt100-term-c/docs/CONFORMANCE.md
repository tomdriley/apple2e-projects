...
| `unobservable` | The real effect cannot be probed on this target. Scored **SKIP** so an untestable claim is never counted as conformance. | nothing (SKIP) |
...
`report-cpr-6n-idempotent` is now `supported`/`spec`: two back-to-back `ESC[6n`
must preserve RAM cursor state and return two exact CPR replies. It formerly
documented issue #24, where synchronous reply gaps corrupted the second query.
`report-da-overlap-lossless` is its discriminating `tolerance` companion: seven
